...
Graph *graph_from_file(FILE *f){
  ...
  if (g->n==0) {
    ...
    if( (g->links=(Link **)malloc(g->n*sizeof(Link*))) == NULL )
      report_error("graph_from_file: malloc() error 3");
    if( (g->links[0]=(Link *)malloc(2*g->m*sizeof(Link))) == NULL )
  ...
  for(i=0;i<g->m;i++) {
    ...
    if ( (u>=g->n) || (v>=g->n) || (u<0) || (v<0) ) {
      ...
      report_error("graph_from_file: bad node numbers or weight");
    }
   ...
   (g->degrees[v]>=g->capacities[v]) ){
    ...
    g->links[u][g->degrees[u]].node = v;
    g->links[u][g->degrees[u]].weight = DEFAULT_WEIGHT;
    g->degrees[u]++;
    // g->links[v][g->degrees[v]].node = u;
    // g->links[v][g->degrees[v]].weight = DEFAULT_WEIGHT;
    // g->degrees[v]++;
  }
  for(i=0;i<g->n;i++)
  ...
  if( fgets(line,MAX_LINE_LENGTH,f) != NULL )
    report_error("graph_from_file; too many lines 1");
...
Graph *wgraph_from_path(const char* path){
  FILE* graph_input = fopen(path,"r");
  if (graph_input == NULL)
    report_error("gwraph_from_file: unable to open file");
  Graph *g = wgraph_from_file(graph_input);
  fclose(graph_input);
  return g;
}
...
Graph *wgraph_from_file(FILE *f){
  ...
  /* read the degree sequence */
  if( (g->capacities=(int *)malloc(g->n*sizeof(int))) == NULL )
    report_error("wgraph_from_file: malloc() error 2");
  if( (g->degrees=(int *)calloc(g->n,sizeof(int))) == NULL )
    report_error("wgraph_from_file: calloc() error");
  for(i=0;i<g->n;i++){
    if( fgets(line,MAX_LINE_LENGTH,f) == NULL )
      report_error("wgraph_from_file; read error (fgets) 2");
    if( sscanf(line, "%d %d\n", &v, &(g->capacities[i])) != 2 )
      report_error("wgraph_from_file; read error (sscanf) 2");
    // printf("\n%d %d\n", v, g->capacities[i]);
    if( v != i ){
      fprintf(stderr,"Line just read : %s\n i = %d; v = %d\n",line,i,v);
      report_error("wgraph_from_file: error while reading degrees");
    }
  }
  ...
  if (g->n==0) {
    ...
    if( (g->links[0]=(Link *)malloc(2UL*g->m*sizeof(Link))) == NULL )
      report_error("wgraph_from_file: malloc() error 4");
    for(i=1;i<g->n;i++)
      g->links[i] = g->links[i-1] + g->capacities[i-1];
  }
  ...
  for(i=0;i<g->m;i++) {
    if( fgets(line,MAX_LINE_LENGTH,f) == NULL )
      report_error("wgraph_from_file; read error (fgets) 3");
    if( sscanf(line, "%d %d %lf\n", &u, &v, &w) != 3 ){
      // fprintf(stderr,"Attempt to scan link #%d failed. Line read:%s\n", i, line);
      report_error("wgraph_from_file; read error (sscanf) 3");
    }
    if ( (u>=g->n) || (v>=g->n) || (u<0) || (v<0) || (w<0.0) ) {
      fprintf(stderr,"Line just read: %s",line);
      report_error("wgraph_from_file: bad node numbers or weight");
    }
    if ( (g->degrees[u]>=g->capacities[u]) ) { 
      // || (g->degrees[v]>=g->capacities[v]) ){
      fprintf(stderr, "reading link %s\n", line);
      report_error("wgraph_from_file: too many links for a node");
    }
    // printf("-> %d %d %d %d\n", u, v, g->degrees[u], g->capacities[i]);
    g->links[u][g->degrees[u]].node = v;
    g->links[u][g->degrees[u]].weight = w;
    g->degrees[u]++;
    // printf("<- %d %d %d\n", u, v, g->capacities[i]);
    // g->links[v][g->degrees[v]].node = u;
    // g->links[v][g->degrees[v]].weight = w;
    // g->degrees[v]++;
  }
  for(i=0;i<g->n;i++)
    if (g->degrees[i]!=g->capacities[i]) {
      printf("%d %d %d\n", i, g->degrees[i], g->capacities[i]);
      report_error("wgraph_from_file: capacities <> degrees");
    }
  if( fgets(line,MAX_LINE_LENGTH,f) != NULL )
    report_error("wgraph_from_file; too many lines 2");
